...
var isPalindrome = function (s) {
  ...
  while (head < tail) {
    //앞뒤를 비교하다가 만약 다르면
    if (s[head] !== s[tail] && skip === 0) {
      return false;
    }
    //skip기회를 뒤쪽을 빼야함. 그래야 -1되고 바로 false 반환을 막을 수 있다.
    if (s[head] !== s[tail]) {
      skip -= 1;
    }
    console.log(head, tail, s[head], s[tail], skip);
    //그렇지 않다면
    //앞+1 , 뒤-1씩하고
    head++;
    tail--;
